	...
	public static Medicament rechercher(String codeMedecin){
		...
		try {
			if (reqSelection.next()) {
				unMedicament = new Medicament(reqSelection.getString(1), reqSelection.getString(2), reqSelection.getString(3), reqSelection.getString(4),reqSelection.getFloat(5), reqSelection.getString(6), reqSelection.getString(7));	
			};
			}
		catch(Exception e) {
			System.out.println("erreur reqSelection.next() pour la requête - select * from MEDICAMENT where MED_DEPOTLEGAL ='"+medDepotLegal+"'");
			e.printStackTrace();
			}
